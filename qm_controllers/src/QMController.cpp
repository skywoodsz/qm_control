...
void QMController::update(const ros::Time &time, const ros::Duration &period) {
    ...
    // Update EE force
    Eigen::Vector3d eeForce = measuredRbdState.segment<3>(2 * qmInterface_->getCentroidalModelInfo().generalizedCoordinatesNum + 7);
    qmInterface_->getEeForcePtr()->setSharedValue(eeForce);
//    std::cout.precision(2);
//    std::cout<<qmInterface_->getEeForcePtr()->getSharedValue().transpose()<<std::endl;
