...
namespace qm{
KalmanFilterEstimate::KalmanFilterEstimate(ocs2::PinocchioInterface pinocchioInterface, ocs2::CentroidalModelInfo info,
        ...
        tfListener_(tfBuffer_), topicUpdated_(false) {
    xHat_.setZero();
    ps_.setZero();
    vs_.setZero();
    a_.setZero();
    ...
    sub_ = ros::NodeHandle().subscribe<nav_msgs::Odometry>("/camera/odom/sample", 10, &KalmanFilterEstimate::callback, this);
}
...
void KalmanFilterEstimate::updateFromTopic() {
    ...
    if (world2odom_.getRotation() == tf2::Quaternion::getIdentity())  // First received
    {
        ...
        try {
            geometry_msgs::TransformStamped tf_msg = tfBuffer_.lookupTransform("odom", msg->child_frame_id, ros::Time(0), ros::Duration(0.1));
    ...
    try {
        geometry_msgs::TransformStamped tf_msg = tfBuffer_.lookupTransform("base", msg->child_frame_id, ros::Time(0), ros::Duration(0.1));
